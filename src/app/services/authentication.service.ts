...
export class AuthenticationService {
  ...
  createUser(name : string, lastName : string, email : string, username : string, password : string, gender : string, city : string) : Observable<any>{
    let user = new User(name, lastName, email, username, password, gender, city, 0);
  ...
  getPrincipals () : Observable<any>  {
    return this.http.get('/api/public/user')
      .map(response => {
        ...
        this.newUser.next(response.json().name);
